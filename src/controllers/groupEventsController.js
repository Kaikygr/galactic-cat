/**
 * @fileoverview Controlador de eventos relacionados a grupos do WhatsApp,
 * especificamente atualizações de participantes (entrada, saída, promoção, rebaixamento).
 * Inclui lógica para enviar mensagens de boas-vindas/despedida personalizadas com mídia
 * e substituição de placeholders com dados do grupo.
 * @requires ../utils/logger - Módulo de logging.
 * @requires ../database/processDatabase - Funções para interagir com o banco de dados.
 * @requires axios - Cliente HTTP para buscar mídias externas.
 */
...
// Mensagem padrão de despedida.
const DEFAULT_EXIT_MESSAGE = "Até mais, {user}! Sentiremos sua falta. 👋";
// Valor padrão para campos nulos ou indefinidos nas mensagens.
const DEFAULT_NULL_VALUE = "não informado";

/**
 * Função auxiliar para obter um valor de um objeto de forma segura,
 * retornando um valor padrão se o caminho não existir ou o valor for null/undefined.
 * @param {object|null} obj - O objeto de onde extrair o valor.
 * @param {string} path - O caminho para o valor (ex: 'property.nestedProperty').
...
const safeGet = (obj, path, defaultValue = DEFAULT_NULL_VALUE) => {
  // Divide o caminho em chaves individuais.
  const keys = path.split(".");
  // Usa reduce para navegar pelo objeto. Começa com o objeto inicial.
  let result = obj;
  for (const key of keys) {
    // Se o objeto atual é nulo/indefinido ou não é um objeto, ou a chave não existe, interrompe.
    if (result == null || typeof result !== "object" || !(key in result)) {
      return defaultValue; // Retorna o padrão se o caminho for inválido em algum ponto.
    }
    // Move para o próximo nível do objeto.
    result = result[key];
  }
  // Se o valor final for null ou undefined, retorna o padrão, senão retorna o valor encontrado.
